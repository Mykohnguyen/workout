...
export class UpdateworkoutComponent implements OnInit {
  ...
  updateWorkout(event){
    let test = [];
    for(let x = 0 ; x < Math.floor(event.target.length/4);x++){
      let newobj = {"name":event.target[x*4].value,"set":event.target[x*4+1].value, "rep":event.target[x*4+2].value, "pounds":event.target[x*4+3].value}
      test.push(newobj)
    }
